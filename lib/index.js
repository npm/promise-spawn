...
const spawnWithShell = (cmd, args, opts, extra) => {
  ...
  if (opts.isWSL) {
    ...
    realArgs = ['-Command']
    const escapedArgs = args.map(arg => `"$(wslpath -w '${arg.replace(/'/g, "''")}')"`)
    script = `Start-Process ${escapedArgs.join(' ')}`
    realArgs.push(script)
  } else if (command === true) {
    // istanbul ignore next
    command = process.platform === 'win32' || opts.isWSL ? process.env.ComSpec || 'cmd.exe' : 'sh'
  }
